...
func readAppTile(d *schema.ResourceData, meta interface{}) error {
	marketplaceClient := meta.(*client.LambdaClient).Marketplace()
	id := d.Id()
	var app *client.AppTileModule
	retryCount := 2
	for app == nil && retryCount > 0 {
		inner, err := marketplaceClient.GetAppTileModule(id)
		app = inner
		if err != nil {
			return err
		}
...
func createAppTile(d *schema.ResourceData, meta interface{}) error {
	...
	id, err := marketplaceClient.PublishNewAppTileModule(client.AppTileCreate{
...
func updateAppTile(d *schema.ResourceData, meta interface{}) error {
	...
	_, err := marketplaceClient.PublishNewAppTileModule(client.AppTileCreate{
		Name:           d.Get("name").(string),
		Image:          d.Get("image").(string),
		AppTileId:      d.Get("app_tile_id").(string),
		Description:    d.Get("description").(string),
